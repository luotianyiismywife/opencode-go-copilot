...
import { normalizeCookie, discoverGoWorkspace, fetchOrCreateApiKey, createApiKey } from '../out/authCookie.js';
...
// ── Minimal fetch helper (for verify + delete steps only) ──
...
async function fetchText(url, cookie) {
    ...
    if (resp.status === 401 || resp.status === 403) throw new Error('cookie-rejected');
    if (!resp.ok) throw new Error(`HTTP ${resp.status}: ${url}`);
    return resp.text();
}

// ── Delete key (DELETE_SERVER_ID not in authCookie.ts) ──
...
async function main() {
    ...
    if (!rawCookie) { console.error('✗ No cookie provided.'); process.exit(1); }
